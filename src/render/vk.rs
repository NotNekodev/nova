...
fn init_imgui(window: &PWindow) -> (imgui::Context, ImguiGLFWSupport) {
    let mut imgui = imgui::Context::create();
    imgui.set_ini_filename(None);

    let platform: ImguiGLFWSupport = ImguiGLFWSupport::init(&mut imgui, window);
